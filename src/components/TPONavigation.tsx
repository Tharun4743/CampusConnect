  ...
  return (
    <>
      <header className="lg:hidden bg-white text-gray-800 h-16 px-4 flex items-center justify-between sticky top-0 z-50 border-b border-gray-200 shadow-xs">
        ...
        <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="p-2 rounded-lg hover:bg-gray-50 text-amber-500">
      ...
      <aside className={`fixed left-0 top-0 h-screen w-72 flex flex-col bg-white border-r border-gray-200 z-40 overflow-y-auto overflow-x-hidden transform lg:static lg:h-auto lg:translate-x-0 transition-transform duration-200 ease-in-out ${mobileMenuOpen ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <div className="px-4 py-3 mx-4 my-4 bg-amber-50 border border-amber-200 rounded-xl flex items-center gap-3">
          ...
          {navItems.map((item) => {
            ...
            return (
              <div key={item.to} className="px-3 mb-1">
                <Link to={item.to} onClick={() => setMobileMenuOpen(false)} className={`flex items-start gap-3 px-3 py-2.5 rounded-lg transition-colors ${isActive ? "bg-amber-50 text-amber-600 border-l-4 border-amber-500" : "text-gray-600 hover:bg-gray-50 border-l-4 border-transparent"}`}>
