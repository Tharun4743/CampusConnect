  ...
  return (
    <>
      <header className="lg:hidden bg-white text-gray-800 h-14 px-3 sm:px-4 flex items-center justify-between sticky top-0 z-50 border-b border-gray-200 shadow-xs shrink-0">
        ...
        <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="p-2 rounded-lg hover:bg-gray-50 text-emerald-500 shrink-0" aria-label={mobileMenuOpen ? "Close menu" : "Open menu"}>
      ...
      <aside className={`sidebar fixed left-0 top-14 lg:top-0 h-[calc(100dvh-3.5rem)] lg:h-auto w-[min(18rem,85vw)] lg:w-72 flex flex-col bg-white border-r border-gray-200 z-40 overflow-y-auto overflow-x-hidden transform lg:static lg:translate-x-0 transition-transform duration-200 ease-in-out ${mobileMenuOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
        ...
        <div className="px-3 py-2.5 mx-3 my-3 lg:px-4 lg:py-3 lg:mx-4 lg:my-4 bg-emerald-50 border border-emerald-200 rounded-xl flex items-center gap-2.5 lg:gap-3">
          ...
          {navItems.map((item) => {
            ...
            return (
              <div key={item.to} className="px-3 mb-1">
                <Link to={item.to} onClick={() => setMobileMenuOpen(false)} className={`flex items-start gap-2.5 lg:gap-3 px-2.5 lg:px-3 py-2 lg:py-2.5 rounded-lg transition-colors ${isActive ? "bg-emerald-50 text-emerald-600 border-l-4 border-emerald-500" : "text-gray-600 hover:bg-gray-50 border-l-4 border-transparent"}`}>
