...
export default function ResetPasswordPage() {
  ...
  const onSubmit = async (data: any) => {
    if (!email) {
      toast.error("Invalid reset session. Please return to forgot password page.");
      return;
    }
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message || "An error occurred. Password reset failed."
      );
    }
  };
      ...
      <div className="max-w-md w-full space-y-8 bg-white p-8 rounded-2xl shadow-sm border border-slate-100">
        <div>
          ...
          <div className="text-center mt-4">
            <div className="mx-auto h-12 w-12 bg-emerald-100 text-emerald-600 rounded-xl flex items-center justify-center mb-4">
            ...
            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-slate-100 text-slate-700 mt-2">
              <Mail className="w-3.5 h-3.5 text-slate-500" />
              {email || "unknown profile session"}
            </span>
          </div>
        </div>
        ...
        <form className="mt-6 space-y-6" onSubmit={handleSubmit(onSubmit)}>
          ...
          >
            {isSubmitting ? (
              ...
              <span>Save Credentials & Return</span>
            )}
          </button>
        </form>
      </div>
