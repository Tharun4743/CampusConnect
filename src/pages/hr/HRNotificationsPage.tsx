      ...
      <main className="app-main overflow-y-auto overflow-x-hidden p-4 sm:p-6 lg:p-8 space-y-6 overflow-y-auto max-w-5xl mx-auto w-full">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">Candidate updates, interview alerts and hiring notifications</p>
          </div>
          {unreadCount > 0 && (
            <button
              onClick={markAllRead}
              className="flex items-center gap-2 text-sm font-medium text-emerald-600 hover:text-emerald-700 px-3 py-2 rounded-lg hover:bg-emerald-100 transition-colors"
            >
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-xl border border-emerald-100 p-12 text-center">
            ...
            <p className="text-sm text-gray-400 mt-1">You'll be notified about candidate activity, interviews and offers here.</p>
          </div>
        ) : (
            ...
            {notifications.map(n => (
              <div
                ...
                className={`bg-white rounded-xl border p-4 flex gap-4 cursor-pointer transition-all ${
                  n.is_read ? "border-gray-100 opacity-70" : "border-emerald-200 shadow-sm hover:shadow-md"
                }`}
              >
                <div className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 ${
                  n.is_read ? "bg-gray-100" : "bg-emerald-50 border border-emerald-200"
                }`}>
                  {typeIcon(n.type)}
                </div>
