  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 font-sans p-6 lg:p-8">
      <div className="max-w-5xl mx-auto">
        ...
        {/* Title banner */}
        <div className="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-6 pb-2 border-b border-slate-800 pb-6">
          <div>
            <div className="flex items-center gap-2 mb-2">
              <span className="px-2.5 py-1 text-[11px] font-bold uppercase tracking-widest bg-indigo-500/10 text-indigo-400 rounded-md border border-indigo-500/20">
                Applications Tracker
              </span>
              <span className="text-xs text-slate-500 font-mono">Real-time candidate profile parsing</span>
            </div>
            <h1 className="text-3xl font-extrabold text-white tracking-tight truncate max-w-xl leading-none">
              {jobTitle} Applicants
            </h1>
            <p className="text-sm text-slate-400 mt-2 max-w-xl leading-relaxed">
              Review applicant qualifications, verify Academic CGPAs, download resume assets, and schedule shortlisted interview routes below.
            </p>
          </div>

          <button
            onClick={() => navigate(`/hr/jobs/applicants/${jobId}/ats`)}
            className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-bold transition-all shrink-0 flex items-center gap-1.5 shadow-md shadow-indigo-600/15 cursor-pointer"
          >
            <Sparkles className="w-4 h-4 text-indigo-200" />
            <span>AI Ranked Candidate Analytics</span>
          </button>
        </div>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 text-center space-y-4 font-mono">
            ...
            <p className="text-slate-400 text-xs">Parsing applicant profiles database...</p>
          </div>
        ) : applicants.length === 0 ? (
              ...
              return (
                <div
                  ...
                  className={`bg-slate-950 rounded-2xl border transition-all cursor-pointer shadow-xs ${
                    isExpanded ? "border-indigo-500/60 ring-1 ring-indigo-500/20" : "border-slate-800/85 hover:border-slate-700"
                  }`}
                >
                  {/* Basic summary row */}
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="space-y-1.5">
                            ...
                            <input
                              ...
                              className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs placeholder:text-slate-600 text-slate-200 focus:outline-hidden focus:border-indigo-500 font-mono"
                            />
                          </div>
                          ...
                          <div className="flex items-center gap-2">
                            {app.resume_url === "NOT_UPLOADED" || app.resume_url === "NONE_UPLOADED" ? (
                              ...
                              <a
                                ...
                                target="_blank"
                                rel="noreferrer"
                                className="px-4 py-2 bg-indigo-500/10 hover:bg-indigo-500 hover:text-white text-indigo-400 rounded-lg text-xs font-bold font-mono transition-all flex items-center gap-2 border border-indigo-500/20"
                              >
